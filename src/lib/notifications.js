...
export function buildNotifications(store) {
  ...
  bills.forEach((b) => {
    ...
    if (dueIn >= 0 && dueIn <= 3) items.push({ key: `bill-${b.id}-${cm}`, icon: "bell", bg: "var(--yellowDim)", col: "var(--yellow)", nm: `${b.name}’s knocking — due ${dueIn === 0 ? "today" : dueIn === 1 ? "tomorrow" : `in ${dueIn} days`}`, mt: "Bills", nav: { type: "sub", bill: b } });
  });
  ...
  installments.forEach((i) => {
    ...
    items.push({ key: `inst-${i.id}-${cm}`, icon: "card", bg: "var(--orangeDim)", col: "var(--orange)", nm: `${label} — this month’s payment is due`, mt: `${fmt(i.installmentAmount)} · Installments`, nav: { type: "inst", instId: i.id } });
  });
  // goals reached
  savings.filter((s) => s.status !== "archived").forEach((s) => {
    if (s.goal > 0 && calcGoalSaved(s.id, txns) >= s.goal) items.push({ key: `goal-${s.id}`, icon: "target", bg: "var(--acDim)", col: "var(--ac)", nm: `You smashed it — ${s.name} goal done!`, mt: "Goals", nav: { type: "goal", goalId: s.id } });
  });
  ...
  budgets.filter((b) => b.kind !== "project" && (b.month ? b.month === cm : (!b.startMonth || cm >= b.startMonth))).forEach((b) => {
    ...
    if (spent > limit) items.push({ key: `budget-${b.id}-${cm}`, icon: "layers", bg: "var(--purpleDim)", col: "var(--purple)", nm: `${b.name} budget is over`, mt: `${fmt(spent - limit)} past your ${fmt(limit)} cap`, nav: { type: "budget", budgetId: b.id } });
  });
  ...
  banks.forEach((b) => {
    ...
    if (safe <= b.lowBalanceThreshold) items.push({ key: `bank-low-${b.id}`, icon: "wallet", bg: "var(--redDim)", col: "var(--red)", nm: `${b.name}’s running low`, mt: `${fmt(safe)} left`, nav: { type: "account", bank: b } });
  });
  // backup nudge (informational — never counts as unread)
  items.push({ key: "backup", icon: "download", bg: "var(--blueDim)", col: "var(--blue)", nm: "Your backup misses you", mt: "Keep a recent copy", muted: true, nav: { type: "privacy" } });
