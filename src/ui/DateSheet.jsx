      ...
      <div className="sheet" role="dialog" aria-label="Pick a date">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 4 }}>
          {cells.map((d, i) => {
            ...
            return <div key={i} onClick={() => !future && setSel(ds)} style={{ aspectRatio: "1", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13.5, fontWeight: on ? 800 : 600, borderRadius: 11, cursor: future ? "default" : "pointer", ...(on ? { background: "var(--ac)", color: "var(--onacc)" } : future ? { color: "var(--faint)", opacity: .4 } : { color: isToday ? "var(--acText)" : "var(--text)", background: isToday ? "var(--surface2)" : "transparent" }) }}>{d}</div>;
          })}
        </div>
        <div style={{ display: "flex", justifyContent: "center", marginTop: 14 }}>
          <span onClick={() => { setSel(today()); setCal(today().slice(0, 7)); }} style={{ padding: "8px 16px", borderRadius: 11, fontWeight: 800, fontSize: 13.5, cursor: "pointer", background: sel === today() ? "var(--ac)" : "var(--surface2)", color: sel === today() ? "var(--onacc)" : "var(--text)" }}>Today</span>
        </div>
