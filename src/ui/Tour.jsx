...
export default function Tour({ steps = [], onClose }) {
  ...
  useLayoutEffect(() => {
    ...
    if (!REDUCED) gsap.fromTo(tipRef.current, { opacity: 0, y: below ? 10 : -10 }, { opacity: 1, y: 0, duration: 0.4, delay: 0.18, ease: "power3.out" });
...
// The tour over the Home screen — targets are live elements on Home.
export const HOME_TOUR = [
  { selector: ".hero", icon: "wallet", pad: 4, title: "Your money at a glance", text: "The top shows your balance and what is safe to spend. Swipe it to switch between them." },
  { selector: ".hero .hib", icon: "eye", pad: 6, title: "Hide your numbers", text: "Tap the eye and every amount turns into dots. Tap again to bring them back." },
  { selector: ".bankcard", icon: "wallet", pad: 6, title: "Your accounts", text: "Each account is a card with its balance. Tap one to see its transactions." },
  { selector: ".tile", icon: "target", pad: 6, title: "This month", text: "Your income and spending for the month. Tap it for the full breakdown." },
  { selector: '[aria-label="Add"]', icon: "plus", pad: 10, title: "Add anything", text: "This is where you add money in, money out, and savings. That is the whole loop." },
];
