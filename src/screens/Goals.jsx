...
export default function Goals({ store, back, onAdd, onOpenGoal }) {
  ...
  return (
    <div className="content padnav">
      ...
      {goals.length === 0 ? (
        <div style={{ textAlign: "center", color: "var(--muted)", padding: "40px", fontWeight: 600 }}>No goals yet. Tap + to start saving.</div>
      ) : goals.map((g) => {
        const pct = g.goal > 0 ? Math.min(100, (g.saved / g.goal) * 100) : 0;
        const left = Math.max(0, g.goal - g.saved);
        return (
          <div className="bcard" key={g.id} onClick={() => onOpenGoal?.(g)} style={{ cursor: "pointer" }}>
            <div className="top"><CatTile cat={goalCat(g)} name={g.name} size={42} /><div className="nm">{g.name}</div><div className="pct">{Math.round(pct)}%</div></div>
            <div className="nums"><div className="a tnum">{fmt(g.saved)}</div><div className="b tnum">{left > 0 ? `${fmt(left)} left · ${fmt(g.goal)}` : `Reached · ${fmt(g.goal)}`}</div></div>
            <div className="pbar bar"><i style={{ width: `${pct}%`, background: "linear-gradient(90deg,var(--ac),var(--ac2))" }} /></div>
          </div>
        );
      })}

      {goals.length > 0 && (
        <div className="frozen" style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 8, background: "var(--acDim)", color: "var(--ac)", borderRadius: 14, padding: "12px 14px", fontWeight: 700, fontSize: 13 }}>
