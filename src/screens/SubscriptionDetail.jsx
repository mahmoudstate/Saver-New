...
import { fmt, today, currentMonth, cardGradient, MONTHS, monthName, monthLabel } from "../lib/format.js";
...
export default function SubscriptionDetail({ store, bill: billProp, back, onEdit }) {
  ...
  const renews = stopped ? tr("subd.stopped") : paidThis ? tr("subd.renewedThisMonth") : dueIn == null ? tr("subd.monthly") : dueIn < 0 ? tr("subd.overdue", { n: Math.abs(dueIn) }) : dueIn === 0 ? tr("subd.renewsToday") : dueIn === 1 ? tr("subd.renewsTomorrow") : tr("subd.renewsInDays", { n: dueIn });
  ...
  const undo = (p) => {
    store.setConfirm({
      ...
      onConfirm: () => { if (p.txnId) store.delTxn(p.txnId); store.set("bills", (list) => list.map((b) => (b.id === bill.id ? { ...b, payments: (b.payments || []).filter((x) => x.month !== p.month) } : b))); store.flash({ title: tr("subd.paymentRemoved"), sub: bill.name, color: "var(--muted)" }); },
    });
  };
    ...
    <div className="content padnav">
      <div className="hero" style={{ position: "relative", overflow: "hidden", background: cardGradient(col), color: "#fff" }}>
        ...
        <div style={{ position: "relative", zIndex: 2, display: "flex", alignItems: "center", gap: 13, marginTop: 4 }}>
          ...
          <div><div style={{ fontSize: 17, fontWeight: 800, letterSpacing: -.3 }}>{bill.name}</div><div style={{ fontSize: 12, opacity: .85, fontWeight: 600 }}>{tr("subd.subscriptionMonthly")}</div></div>
        </div>
        <div className="lbl" style={{ color: "rgba(255,255,255,.8)", marginTop: 12 }}>{tr("subd.monthlyLbl")}</div>
        ...
        <div className="sub" style={{ color: "rgba(255,255,255,.85)" }}>{renews}{bill.dueDay && !stopped ? tr("subd.daySuffix", { n: bill.dueDay }) : ""}{tr("subd.fromSuffix", { bank: bankName })}</div>
      </div>

      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 18 }}>
        <span className="chip">{tr("subd.perYear", { amt: fmt(bill.amount * 12) })}</span>
      ...
      {history.length === 0 ? <div style={{ color: "var(--muted)", fontWeight: 600, padding: "12px 2px" }}>{tr("subd.noPaymentsDot")}</div>
        : history.map((p) => (
          <div className="icard" key={p.month} onClick={() => undo(p)} style={{ cursor: "pointer" }}>
            ...
            <div><div className="nm">{monthLabel(p.month)}</div><div className="mt">{tr("subd.paid")}{p.date ? " " + p.date : ""} · {bankName} · {tr("subd.tapUndo")}</div></div>
      ...
      <div className="cta">
        {stopped
          ...
          : <div className="btn btn-primary btn-full" onClick={record} style={{ opacity: paidThis ? .5 : 1 }}><Ico name="check" size={18} />{paidThis ? tr("subd.paidForMonth", { month: monthName(+cm.split("-")[1] - 1) }) : tr("subd.recordPaymentFor", { month: nextMonthLabel })}</div>}
      </div>
