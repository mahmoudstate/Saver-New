...
export default function ProfileEdit({ store, back }) {
  ...
  const save = () => { store.set("username", name.trim()); store.set("currency", cur); store.flash({ title: "Profile saved", sub: name.trim() || "Your name", color: "var(--success)", icon: "check" }); back(); };
