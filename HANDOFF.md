...
**Where we are:** Bills (Subscriptions + Installments) fully reworked + a multi-round design polish on the **New-subscription editor** (Mahmoud reviewed it closely). All pushed. **Project now lives on a NEW remote `Saver-New`, branch `main`** (see git note). A1 · Home done; A5 · Profile + A20 · Category editor done earlier. Resume by continuing the §7 list (A2 · Activity onward) or any new Mahmoud request.
...
> **Current cursor:** Bills + Subscription/Installment editors done and polished over several rounds; old-backup category-edit crash fixed; project moved to `Saver-New/main` (`05f0370`) and the repo was tidied. Next: resume the §7 list (A2 · Activity onward) or whatever Mahmoud asks. Open polish noted by Claude: no "pause" glyph for the Stop menu item (uses `back` arrow); legacy demo bills lack `domain`/`typeId` so they show a monogram + land under "Other" (new ones from the picker are fine). Nothing is ✅ until Mahmoud says so.
